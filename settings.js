...
function updateBetDisplay(n) {
  ...
  if (player[n]) {
    if (player[n].bet != null)
      s = "הימור: " + formatDollar(player[n].bet);
    else
      s = "\u00a0";
  ...
  creditsTextNode.nodeValue = "כסף בבנק:  " + formatDollar(credits);
}
...
function changeBet(n) {
  ...
  defaultTextNode.nodeValue = "הימור התחלתי: " + formatDollar(defaultBet);
